...
do
    counter+=1
    cost_time=counter*5;
    printf "\nPlease wait, Config Server is staring... ...[$cost_time s]"
    sleep 5
done
...
then
    printf "\n$(date) Config Server failed to start in $total_time seconds!"
    printf "\nLog file location: $LOG_DIR/$SERVICE_NAME.log"
    exit 1;
fi

printf "\nConfig Server started on port(s): $SERVER_PORT (http)"
printf "\nStarted Config Server in $total_time seconds!"
printf "\nLog file location: $LOG_DIR/$SERVICE_NAME.log"
