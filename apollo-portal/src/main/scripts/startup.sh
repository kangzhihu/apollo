...
do
    counter+=1
    cost_time=counter*5;
    printf "\nPlease wait, Portal is staring... ...[$cost_time s]"
    sleep 5
done
...
then
    printf "\n$(date) Portal failed to start in $total_time seconds!"
    printf "\nLog file location: $LOG_DIR/$SERVICE_NAME.log"
    exit 1;
fi

printf "\nPortal started on port(s): 8070 (http)"
printf "\nStarted Portal in $total_time seconds!"
printf "\nLog file location: $LOG_DIR/$SERVICE_NAME.log"
